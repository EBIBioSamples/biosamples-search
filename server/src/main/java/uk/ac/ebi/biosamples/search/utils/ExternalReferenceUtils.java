package uk.ac.ebi.biosamples.search.utils;

import org.springframework.web.util.UriComponents;
import org.springframework.web.util.UriComponentsBuilder;
import uk.ac.ebi.biosamples.search.model.ExternalReference;
...
public class ExternalReferenceUtils {
  ...
  public static String getNickname(final ExternalReference externalReference) {
    if (externalReference.url().contains(ENA_BASE_URL_FRAGMENT)) {
      return "ENA";
    } else if (externalReference.url().contains(ARRAYEXPRESS_BASE_URL_FRAGMENT)) {
      return "ArrayExpress";
    } else if (externalReference.url().contains(HPSCREG_URL_FRAGMENT)) {
      return "hPSCreg";
    } else if (externalReference.url().contains(DBGAP_BASE_URL_FRAGMENT)) {
      return "dbGaP";
    } else if (externalReference.url().contains(EGA_DATASET_BASE_URL_FRAGMENT)) {
      return "EGA Dataset";
    } else if (externalReference.url().contains(EGA_SAMPLE_BASE_URL_FRAGMENT)) {
      return "EGA Sample";
    } else if (externalReference.url().contains(EGA_STUDY_BASE_URL_FRAGMENT)) {
      return "EGA Study";
    } else if (externalReference.url().contains(BIOSTUDIES_BASE_URL_FRAGMENT)) {
      return "BioStudies";
    } else if (externalReference.url().contains(EVA_BASE_URL_FRAGMENT)) {
      return "EVA";
    } else {
      return "other";
    }
  ...
  public static Optional<String> getDataId(final ExternalReference externalReference) {
    ...
    if ("ENA".equals(nickname)
      ...
      final UriComponents uriComponents =
          UriComponentsBuilder.fromHttpUrl(externalReference.url()).build();
